...
public class Practice14MaskFilterView extends View {
    ...
    protected void onDraw(Canvas canvas) {
        ...
        // 第一个：NORMAL
        maskFilter = new BlurMaskFilter(40, BlurMaskFilter.Blur.NORMAL);
        paint.setMaskFilter(maskFilter);
        canvas.drawBitmap(bitmap, 100, 50, paint);
        paint.reset();

        // 第二个：INNER
        maskFilter = new BlurMaskFilter(40, BlurMaskFilter.Blur.INNER);
        paint.setMaskFilter(maskFilter);
        canvas.drawBitmap(bitmap, bitmap.getWidth() + 200, 50, paint);

        // 第三个：OUTER
        maskFilter = new BlurMaskFilter(40, BlurMaskFilter.Blur.OUTER);
        paint.setMaskFilter(maskFilter);
        canvas.drawBitmap(bitmap, 100, bitmap.getHeight() + 100, paint);

        // 第四个：SOLID
        maskFilter = new BlurMaskFilter(40, BlurMaskFilter.Blur.SOLID);
        paint.setMaskFilter(maskFilter);
